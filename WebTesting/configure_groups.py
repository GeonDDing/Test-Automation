...
class ConfigureGroup(WebDriverMethod):
    def __init__(self):
        ...
        try:
            ...
            if not self.find_exist_group(group_name):
                ...
                self.input_text(By.CSS_SELECTOR,
                                self.group_elements.group_name, group_name)
            else:
                print('A group with the same name exists.')

            # Set group domain
            group_domain_selector = f"{self.group_elements.group_domain}[value='1']" if domain == 'live' else f"{self.group_elements.group_domain}[value='2']"
            ...
            if domain == 'live':
                ...
                self.click_element(
                    By.CSS_SELECTOR, trigger_css_selector)

            # Save group settings
            self.click_element(
                By.CSS_SELECTOR, self.group_elements.group_save_button)
