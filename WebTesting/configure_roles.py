...
class ConfigureRole(WebDriverMethod):
    ...
    def configure_role(self, role_name, *channel_name):
        try:
            ...
            if not self.find_exist_role(role_name):
                self.click_element(
                    By.CSS_SELECTOR, self.role_elements.role_add_button)
                ...
                self.input_text(By.CSS_SELECTOR,
                                self.role_elements.role_name, role_name)
                if channel_name:
                    for index, channel_value in enumerate(channel_name):
                        self.select_element(
                            By.XPATH, self.role_elements.add_channel_list, 'text', channel_value)
            else:
            ...
            self.click_element(
                By.CSS_SELECTOR, self.role_elements.role_save_button)
    ...
    def find_exist_role(self, role_name):
        try:
            role_table = self.find_web_element(
                By.XPATH, self.role_elements.role_table)
