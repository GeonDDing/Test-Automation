...
class ConfigureTask(WebDriverMethod):
    ...
    def configure_task(self, task_name, taks_options=None):
        try:
            ...
            if not self.find_exist_task(task_name):
                self.click_element(
                    By.CSS_SELECTOR, self.task_elements.task_add_button)
                ...
                self.input_text(By.CSS_SELECTOR,
                                self.task_elements.task_name, task_name)
            else:
                print('A task with the same name exists.')
            select_relevant_keys = {'Group', 'Channel', 'Task', 'Replacement Image',
                                    'Replacement Playlist', 'Recurring daily', 'State'}
            for key, value in taks_options.items():
                print(f"  ㆍ{key} : {value}")
                element_selector = getattr(
                    self.task_elements, f"task_{''.join(key.replace(' ', '_').lower().split('-'))}" if '-' in key else f"task_{key.replace(' ', '_').lower()}", None)
            ...
            # Save task settings
            self.click_element(
                By.CSS_SELECTOR, self.task_elements.task_save_button)
    ...
    def find_exist_task(self, task_name):
        try:
            task_table = self.find_web_element(
                By.XPATH, self.task_elements.task_table)
