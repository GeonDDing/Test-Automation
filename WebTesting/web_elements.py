...
class MainMenuElements:
    ...
    configure_groups, configure_devices, configure_roles, configure_channels, \
        configure_video_presets, configure_audio_presets, configure_tasks, \
        configure_file_jobs, configure_watch_folders = [
            f'//*[@id="page-submenu-inner"]/div/a[{i}]' for i in range(1, 10)]

    settings_mgmtsys_settings, settings_events, settings_event_action, \
        settings_maintenance, settings_update, settings_networking, \
...
class ConfigureVideopresetElements():
    ...
    videopreset_buffering_time = 'input[type="text"][id="/video/lookaheaddepth"][name="/video/lookaheaddepth"]'
