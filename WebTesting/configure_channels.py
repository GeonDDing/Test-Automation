...
class Configurechannel(WebDriverMethod):
    ...
    def configure_channel(self, channel_name, input_type, output_type, backup_source_type=None,  input_options=None, output_options=None, backup_source_options=None, profile_name=None):
        try:
            ...
            configure_output = ConfigureOutput(
                output_type, profile_name['Videopreset Name'], profile_name['Audiopreset Name'])
            ...
            self.input_text(By.CSS_SELECTOR,
                            self.channel_elements.channel_name, channel_name)
            ...
            try:
                error_message = self.find_web_element(
                    By.CSS_SELECTOR, self.input_elements.input_common_error_message).get_attribute('innerText')
            ...
            if not backup_source_type == None:
                configure_backup_source = ConfigureBackupSource(
                    backup_source_type)
                self.navigate_to_configure_channels()
                time.sleep(1)
                ...
                if backup_source_type in backup_source_functions:
                    backup_source_functions[backup_source_type](
