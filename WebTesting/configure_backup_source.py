...
class ConfigureBackupSource(WebDriverMethod):
    def __init__(self, backup_source_type):
        ...
        try:
            self.select_element(
                By.CSS_SELECTOR, self.backup_source_elements.backup_source_type, 'text', backup_source_type)
        ...
        try:
            ...
            for key, value in backup_source_options.items():
                ...
                element_selector = getattr(
                    self.backup_source_elements, f"input_udp_{''.join(key.replace(' ', '_').lower().split('-'))}" if '-' in key else f"input_udp_{key.lower().replace(' ', '_')}", None)
                if any(keyword in key for keyword in select_relevant_keys):
                    if 'Program Selection Mode' in key:
                        ...
                        if mode in ['Program number', 'Service name']:
                            mapping = {
                                'Program number': (self.backup_source_elements.backup_source_udp_program_number, '1010'),
                                'Service name': (self.backup_source_elements.backup_source_udp_service_name, 'E2 Channel')
                            }
    ...
    def backup_source_rtp(self, backup_source_options):
        try:
            ...
            self.input_text(By.CSS_SELECTOR,
                            self.channel_elements.backup_source_udp_network_url, backup_source_options.get('sdf_file'))
    ...
    def backup_source_rtmp(self, backup_source_options):
        try:
            ...
            self.input_text(By.CSS_SELECTOR,
                            self.channel_elements.backup_source_udp_network_url, backup_source_options.get('url'))
    ...
    def backup_source_playlist(self, backup_source_options):
        try:
            ...
            if playlist_type:
                self.select_element(
                    By.CSS_SELECTOR, self.channel_elements.input_playlist_type, 'text', playlist_type)
                if playlist_type == 'Local Static Playlist':
                    self.select_element(
                        By.CSS_SELECTOR, self.channel_elements.input_playlist_playlists_name)
                elif playlist_type == 'Clipcasting XML' or playlist_type == 'Remote Media Asset Directory':
                    self.input_text(
                        By.CSS_SELECTOR, self.channel_elements.input_playlist_clipcasting_uri, backup_source_options.get('uri'))
                    if playlist_type == 'Remote Media Asset Directory':
                        self.input_text(
                            By.CSS_SELECTOR, self.channel_elements.input_playlist_clipcasting_uri, backup_source_options.get('recurring'))
                        self.select_element(
                            By.CSS_SELECTOR, self.channel_elements.input_playlist_type, 'text', backup_source_options.get('sort_by'))
            time.sleep(1)
    ...
    def backup_source_smpte_st_2110(self, backup_source_options):
        try:
            ...
            for key, value in backup_source_options.items():
                ...
                if element_selector:
                    if key in ['video_sdp_url', 'audio_sdp_url', 'ancillary_sdp_url', 'teletext_page']:
                        self.input_text(By.CSS_SELECTOR,
                                        element_selector, value)
                else:
                    self.select_element(
