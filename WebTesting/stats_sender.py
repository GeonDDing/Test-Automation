...
class StatsSender():
    def __init__(self):
        try:
            script_directory = os.path.dirname(
                os.path.realpath(__file__))  # 현재 경로만 추출
            config_path = os.path.join(
                script_directory, 'config.ini')  # 현재 경로에서 config.ini 찾음
            config = configparser.ConfigParser()  # config.ini 파싱
            config.read(config_path)  # config.ini 파싱 후 일기
    ...
    def stats_sender(self, queue):
        output_info = None
        while True:
                # 지금은 channel index 를 prameter로 입력하지만 추후 실행 중인 채널을 자동으로 찾을 수 있도록 변경
            try:
                chidx = 0
                stat_response = requests.get(
                    f"{self.url}:900{chidx}/stats")
            ...
            # print((chidx, source_layer, source_stat, output_info))
