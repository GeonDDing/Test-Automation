        ...
        try:
            ...
            if not self.find_exist_device(device_name):
                ...
                self.input_text(By.CSS_SELECTOR,
                                self.device_elements.device_name, device_name)
            else:
                print('A device with the same name exists.')

            # IP Address
            self.input_text(By.CSS_SELECTOR,
                            self.device_elements.device_ip_address, ip_address)
            # Select Group
            self.select_element(
                By.CSS_SELECTOR, self.device_elements.device_include_group, 'text', group_name)
            # Select Role
            self.select_element(
                By.CSS_SELECTOR, self.device_elements.device_include_role, 'text', role_name)
            # Save Device settings
            self.click_element(
                By.CSS_SELECTOR, self.device_elements.device_save_button)
