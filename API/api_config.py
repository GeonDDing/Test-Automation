    ...
    def convert_json(self, json_file, http_method):
        ...
        try:
            file_path = os.path.join(
                os.getcwd(), 'api', 'json', f"{http_method}_{json_file}.json")
            ...
            with open(file_path, 'r') as f:
                ...
                if isinstance(json_data, list):
                    data_list = [item['data']
                                 for item in json_data if 'data' in item]
...
if __name__ == '__main__':
    config = ApiConfig()
    # config.convert_json('videopresets', 'post')
    config.convert_json('audiopresets', 'put')
