...
def perform_api_operations(api_operation, operation_name):
    ...
    for i, response in enumerate(response_post):
        attach_response_step(f'POST {operation_name}',
                             response, f"POST Response Status Code {i+1}", f"POST Response Result {i+1}")

    # GET after POST request
    response_get = api_operation.get_api_operation(
        None, 'devid', '1', 'chindex', '1')
...
def test_id3s():
    ...
    control_response = requests.put(f"{ApiOperation('controls').api_url}?id=1&chidx=1", headers=ApiOperation(
        'controls').headers, data=json.dumps(channel_start))
    if control_response.status_code == 200:
        print('Channel started successfully')
        sleep(10)
    api_operation = ApiOperation('id3s')
    perform_api_operations(api_operation, 'id3')
    requests.put(f"{ApiOperation('controls').api_url}?id=1&chidx=1", headers=ApiOperation(
        'controls').headers, data=json.dumps(channel_stop))
